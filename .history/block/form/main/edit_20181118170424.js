...
const getSingleSlideTemplate = memoize(singleSlide => {
  return times(singleSlide, n => [
    "block-slider/block-single-slide",
    { id: n + 1 }
  ]);
});
        ...
        <InspectorControls>
          <PanelBody>
            <TextControl
              ...
              onChange={width => setAttributes({ width:Number(width) })}
            />
            <TextControl
              type="number"
              label={__("Height")}
              value={attributes.height}
              onChange={(height) => setAttributes({ width:Number(height) })}
            />
            <ToggleControl
              label={__("Navigation")}
